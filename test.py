...
from basic_layers import LSTM, FeedForward, ConnectLayers, SparseLSTM
...
def batches2string(batches):
    ...
    s = [''] * batches[0].shape[0]
    for b in batches:
        s = [''.join(x) for x in zip(s, ids2str(b))]
    return s
...
with graph.as_default():
    lstm = SparseLSTM(vocabulary_size, num_nodes, batch_size)
    linear = FeedForward(num_nodes, vocabulary_size, batch_size)
    model = ConnectLayers((lstm, linear))
    ...
    with tf.control_dependencies([lstm.save_state()]):
        # Classifier.
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        if step % summary_frequency == 0:
            ...
            onehot_labels = onehot(labels, vocabulary_size)
            print('Minibatch perplexity: %.2f' % float(
                np.exp(logprob(predictions, onehot_labels))))
            ...
            for _ in range(valid_size):
                ...
                valid_logprob = valid_logprob + logprob(predictions, onehot(b[1], vocabulary_size))
